...
def detect_align_embed_studio_faces(path, mtcnn_model, margin=0):
    ...
    try:
        ...
        if boxes is None:
            cur.execute("""
            SELECT COUNT(*) FROM TestPlayerPhotoEmbedding
            ...
            if result[0] > 0:
                ...
                cur.execute("""
                    INSERT INTO TestPlayerPhotoEmbedding (RosterID, TournamentID, SFaceEmbeddings, ImagePath, TeamKey, UpdatedOn, CreatedOn, ConstellationID)
    ...
    if queue_row_count == 0:
        ...
        try:
            ...
            for test_game in test_games:
                ...
                print('roster_rows:', total_studio_images)
                ...
                for roster_row in tqdm(roster_rows, desc="Processing Studio Images", unit="image", total=total_studio_images):
                    ...
                    for embedding in photo_embeddings:
                        ...
                        if result[0] > 0:
                            ...
                            cur.execute("""
                                INSERT INTO TestPlayerPhotoEmbedding (RosterID, TournamentID, SFaceEmbeddings, ImagePath, TeamKey, UpdatedOn, CreatedOn, ConstellationID, SBoundingBox)
                            ...
                            cnxn.commit()
                # Below query checks if there are already game photo embeddings exist in the TestGamePhotoEmbedding table, if found, it deletes them.
                cur.execute("DELETE FROM TestGamePhotoDetail WHERE GameNumber = ?", (test_game,))
                cur.execute("DELETE FROM TestGamePhotoEmbedding WHERE GameNumber = ?", (test_game,))
                ...
                for game_row in tqdm(game_rows, desc="Processing Game Images", unit="image", total=total_game_images):
                    ...
                    id = cur.execute("""
                        INSERT INTO TestGamePhotoDetail (NuFaceDetected, ImagePath, RosterIDs, GameNumber, TournamentID, TournamentWeek, TournamentWeekNumber, GameDay, GameTime, GameField, GameDayNumber, GameTimeNumber, GameType, U, B, R, ConstellationID, IsMatch, IsGroupPhoto, IsTagged, IsAttempted, UpdatedOn, CreatedOn, HiresWidth, HiresHeight) OUTPUT inserted.TestGamePhotoDetailID
                    ...
                    if isgamephotonull is True:
                        NonHuman += 1
                        cur.execute("""
                            INSERT INTO TestGamePhotoEmbedding (TestGamePhotoDetailID, GFaceEmbedding, UpdatedOn, CreatedOn, GameNumber)
                            VALUES (?, NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?)
                        """, (id, game_data['GameNumber']))
                        cnxn.commit()
                        isgamephotonull = False
                    # For each game photo, adding the embedding and TestGamePhotoDetailid into the TestGamePhotoEmbedding table
                    for embedding in photo_embeddings:
                        game_photos += 1
                        cur.execute("""
                            INSERT INTO TestGamePhotoEmbedding (TestGamePhotoDetailID, GFaceEmbedding, UpdatedOn, CreatedOn, GameNumber, GBoundingBox)
                            VALUES (?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?, ?)
                    ...
                    #                 cur.execute("""UPDATE TestGamePhotoDetail SET JerseyNumbers = COALESCE(JerseyNumbers + ', ', '') + ?  WHERE ImagePath = ?""", (text, game_image_path))
                    #                 cnxn.commit()  
                    ...
                    #                 continue
                        ...
                        #     print("Error:", e)
                cur.execute("""SELECT TestPlayerEmbeddingId, SFaceEmbeddings, RosterID FROM TestPlayerPhotoEmbedding WHERE TeamKey IN (?, ?) AND SFaceEmbeddings is Not Null""", (str(key_data["HomeTeamKey"]), str(key_data["VisitorTeamKey"])))
                ...
                euclidean_threshold = 1.5 - (float(queue_data['Threshold']) / 100) * (1.2 - 0.6)
                print('euclidean_threshold:', euclidean_threshold)
                for player in player_embeddings:
                    ...
                    cur.execute("""
                        SELECT TestGamePhotoEmbeddingId, GFaceEmbedding, TestGamePhotoDetailId 
                        FROM TestGamePhotoEmbedding 
                    ...
                    for game in game_embeddings:
                        ...
                        if distance < euclidean_threshold:
                            match_count+=1
                            cur.execute("""
                                        UPDATE TestGamePhotoDetail
                                        SET MatchedFaces = MatchedFaces + 1,
                                            RosterIDs = COALESCE(RosterIDs + ' ', '') + ?
                                        WHERE TestGamePhotoDetailId = ?
                                    """, (str(player['RosterID']), game['TestGamePhotoDetailId']))
                            cnxn.commit()  
                matched_count.append(match_count)
                total_count.append(game_photos)
            matched = sum(matched_count)
            #print('matched:', matched)
            photo_total = sum(total_count)
            #print('photo_total:', photo_total)
            total = sum(total_count)-int(NonHuman)
            #print('total:', total)
            ...
            cur.execute("""
                    ...
                    """, (total, NonHuman, matched, percentage, queue_data['TestMatchingConfigId']))
            cnxn.commit()
